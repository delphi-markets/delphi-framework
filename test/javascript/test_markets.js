...
contract('Market', function (accounts) {
    ...
    beforeEach(async () => {
        ...
        arbiter = getParamFromTxEvent(
            await arbiterFactory.createArbiter(ipfsHash),
            'arbiter', Arbiter
        )
        event = getParamFromTxEvent(
            await eventFactory.createCategoricalEvent(etherToken.address, arbiter.address, 2),
            'categoricalEvent', Event
        )
    })

    it('test market creation and market closing', async () => {
        // Create market
        const buyer = 5
    ...
    it('test allow buying and selling', async () => {
        // create market
        const investor = 0
    ...
    it('test allow short selling', async () => {
        // create market
        const investor = 7
    ...
    it('test creation by a successful campaign', async () => {
        // Create campaign
        const feeFactor = 50000  // 5%
        const funding = 1e18
    ...
    it('test creation-failure by unsuccessful campaign', async () => {
        // Create campaign
        const feeFactor = 50000  // 5%
        const funding = 1e18
        const deadline = web3.eth.getBlock('latest').timestamp + 60  // in 1h
