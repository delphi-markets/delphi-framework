...
class TestContracts(AbstractTestContracts):
    ...
    def test(self):
        ...
        signatory_1 = 0
        signatory_2 = 1
        v, r, s = self.sign_data(description_hash, keys[signatory_1])
        oracle = self.contract_at(self.signed_message_oracle_factory.createSignedMessageOracle(description_hash, v, r, s),
                                  self.oracle_abi)
        self.assertEqual(oracle.signatory(), encode(accounts[signatory_1], 'hex'))
        # Replace signatory
        nonce = 1
        replace_hash = sha3(
            description_hash +
            accounts[signatory_2] +
            self.i2b(nonce)
        )
        # Signing by registered signatory works
        v, r, s = self.sign_data(replace_hash, keys[signatory_1])
        oracle.replaceSignatory(accounts[signatory_2], nonce,  v, r, s, sender=keys[signatory_1])
        self.assertEqual(oracle.signatory(), encode(accounts[signatory_2], 'hex'))
        # Set outcome
        outcome = 1
        result_hash = sha3(
            description_hash +
        ...
        # Signed by wrong signatory fails
        v, r, s = self.sign_data(result_hash, keys[signatory_1])
        self.assertRaises(TransactionFailed, oracle.setOutcome, outcome, v, r, s)
        self.assertFalse(oracle.isOutcomeSet())
        # Signing by registered signatory works
        v, r, s = self.sign_data(result_hash, keys[signatory_2])
